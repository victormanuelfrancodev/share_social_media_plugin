...
class Tweet {
  ...
  String? _formatText(String? full) {
    String? addLinks(String? formatted) {
      final matches = RegExp("https?://(www\\.)?[-a-zA-Z0-9@:%._+~#=]{2,256}\\.[a-z]{2,6}\\b([-a-zA-Z0-9@:%_+.~#?&/=]*)", caseSensitive: false).allMatches(full!);
      ...
      for (final match in matches) {
        ...
        if (link.length > 40) {
          ...
          else if (link.startsWith("http://")) link = link.substring("http://".length);
        ...
        formatted = formatted!.replaceRange(start, end, '<a href="$fullLink">$link</a>');
      }
    ...
    String? addMentions(String? formatted) {
      final matches = RegExp("(@[A-Za-z0-9_]+)", caseSensitive: false).allMatches(full!);
      ...
      for (final match in matches) {
        ...
        formatted = formatted!.replaceRange(start, end, '<a href="$fullLink">$fullLink</a>');
      }
